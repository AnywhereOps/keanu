/**
 * Keanu IDE MCP server.
 * Exposes openDiff/closeDiff tools and sends IDE context over JSON-RPC 2.0.
 */
...
const IDE_SERVER_PORT_ENV_VAR = 'KEANU_IDE_SERVER_PORT';
const IDE_WORKSPACE_PATH_ENV_VAR = 'KEANU_IDE_WORKSPACE_PATH';
const IDE_AUTH_TOKEN_ENV_VAR = 'KEANU_IDE_AUTH_TOKEN';
    ...
    return new Promise((resolve) => {
      ...
      this.server = app.listen(0, '127.0.0.1', async () => {
        ...
        if (address && typeof address !== 'string') {
          ...
          try {
            const portDir = path.join(os.tmpdir(), 'keanu', 'ide');
            await fs.mkdir(portDir, { recursive: true });
            portFile = path.join(
              portDir,
              `keanu-ide-server-${process.ppid}-${this.port}.json`,
            );
...
const createMcpServer = (
  ...
  const server = new McpServer(
    {
      name: 'keanu-ide-companion-mcp-server',
      version: '1.0.0',
    },
