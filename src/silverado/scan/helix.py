...
def helix_scan(lines, threshold=0.45,
    ...
    try:
        ...
        print("  collection 'silverado_rgb' not found. run: silverado bake", file=sys.stderr)
        return None
...
def _mood_from_detect(r_pos, r_neg, y_pos, y_neg, b_pos, b_neg):
    ...
    try:
        from silverado.detect.mood import detect
        reading = detect(
...
def _mood_fallback(r_pos, r_neg, y_pos, y_neg, b_pos, b_neg):
    ...
    if fires == 3 and min(r_net, y_net, b_net) > 0.3:
        ...
        if max(r_neg, y_neg, b_neg) > 0.5:
            state, symbol = "Black", "💀"
            nudge = "multiple fires collapsed. stop."
        else:
            state, symbol = "Grey", "⬜"
            nudge = "nobody home. performing."
