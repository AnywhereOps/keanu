...
from silverado.compress.dns import ContentDNS, sha256
from silverado.compress.instructions import COEFInstruction, COEFProgram
from silverado.detect.mood import detect, scan_text, SynthesisReading
...
class COEFExecutor:
    ...
    def _score_step(self, inst, dns_miss, no_change, before, after):
        if dns_miss:
            event = "dns_miss: operating blind"
        elif no_change:
            event = "no_change: silent failure"
        else:
            event = f"ok: {inst.op}"
